...
export default function DeletePolicyAcknowledgmentSection({ data }) {
    ...
    const deleteAcknowledgment = async (e) => {
        e.preventDefault();
        setLoading(true);
        try {
            ...
            message.error("Failed to Delete Acknowledgment. Please try again.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <>
            ...
            <Modal open={isModalOpen} setOpen={setIsModalOpen} onClose={() => setIsModalOpen(false)}>
                <h2 className="text-xl mb-4">
                    ...
                    }[data?.type] || data?.type}</b> acknowledgment of <b>{data?.name}</b>?
                </h2>
                <form action="" onSubmit={deleteAcknowledgment}>
